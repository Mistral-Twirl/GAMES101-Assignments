...
void Application::init() {
  ...
  ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 16, config.mass,
                       config.ks, {0});
  ropeSemiImplicit = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 16, config.mass,
                       config.ks, {0});
...
void Application::render() {
  ...
  for (int i = 0; i < config.steps_per_frame; i++) {
    ...
    ropeSemiImplicit->simulateSemiImplicit(1 / config.steps_per_frame, config.gravity);
